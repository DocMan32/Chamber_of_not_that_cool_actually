...
public:
	...
	// Coffee gives a +1 modifier onto selected target
	int coffee;

	// Zombie Monkey Paw, costs more but gives you a 50/50 on getting your selected target an 
	// instant win OR the hand comes to life and steals half your chips
	int ZombieMonkeyPaw;

	// Environmental item, gives chance to hit selected target but due to its small arms and low
	// Accuracy it has a good chance of missing and damaging a random target
	int LaserDinosaur;
...
void BattleGO(Mob& Ganon, Mob& Ganon2, Gambling gambit)
{
	...
	{
		// If statement for death and wagers 
...
int main()
{
	...
	while (combat == 0);
	{
		...
		if (gambit.Wager == "1")
		{
			...
			if (fun > 150)
			{
				std::cout << "You dont have enough\n";
				std::cout << "Now i'll ask again, how much money are you betting? \n";
		...
		else if (gambit.Wager == "2")
		{
			...
			if (bouttamakeaname > 150)
			{
				std::cout << "You dont have enough try again\n";
			}
			else if (bouttamakeaname <= 150)
			{
				combat = 1;
			}
